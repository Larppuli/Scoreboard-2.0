...
import { IconCircleFilled, IconScale } from '@tabler/icons-react';
import { Sparkline } from '@mantine/charts';
import { Badge, Group, Stack, Text } from '@mantine/core';
import { useAppContext } from '@/app/lib/AppContext';
import { PointsCardProps, User } from '@/app/lib/definitions';
...
export default function PointsCard({ pointsArray }: PointsCardProps) {
  ...
  const icon = selectedUser ? (
    <IconCircleFilled size={10} style={{ marginRight: '-10px' }} color={'#ffd100'} />
  ) : (
    <IconScale size={15} />
  );
  ...
  const development = {
    ...
    percent:
      pointsArray[referenceIndex] !== 0
        ? ((pointsArray[pointsArray.length - 1] - pointsArray[referenceIndex]) /
            pointsArray[referenceIndex]) *
          100
        : 0,
  ...
  const formattedPercent =
    development.percent > 0
      ? `+${development.percent.toFixed(2)}%`
      : `${development.percent.toFixed(2)}%`;

  const formattedAmount =
    development.amount > 0 ? `+${development.amount}` : `${development.amount}`;
  ...
  const handleSparklineLengthSelect = (length: number) => {
    ...
    setSelectedUserPointsArray(
      selectedUserSparkinglinePoints.slice(selectedUserSparkinglinePoints.length - length)
    );
  ...
  useEffect(() => {
    const trimmedLength = selectedUser
      ? Math.min(
          findClosestSparklineLength(pointsArray.length),
          findClosestSparklineLength(selectedUserPointsArray.length)
        )
      : findClosestSparklineLength(pointsArray.length);
    setSparklineGameNum(trimmedLength);
    setUserPointsArray(pointsArray.slice(-trimmedLength));
    setMaxSparklineGameNum(
      selectedUser
        ? Math.min(pointsArray.length, selectedUserPointsArray.length)
        : pointsArray.length
    );
  ...
  return (
    <Stack mt="10px" w="80vw" bg={'#141414'} style={{ borderRadius: '8px' }}>
      <Stack gap={0}>
        <Stack gap={0}>
      ...
      <CompareModal
        ...
        setOpened={setCompareOpened}
        onUserSelect={handleUserSelect}
      />
      <SparklineLengthModal
        opened={lengthModalOpened}
        setOpened={setLengthModalOpened}
        onLengthSelect={handleSparklineLengthSelect}
        maxLength={maxSparklineGameNum}
        setDisplayMax={handleDisplayMaxChange}
      />
    </Stack>
  );
}
